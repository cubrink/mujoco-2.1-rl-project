...
class ActorCritic(nn.Module):
    def __init__(self, observation_space, action_space, hidden_size=64, lr=3e-4, gamma=0.99):
        super().__init__()
    ...
    def update(self):
        ...
        critic_loss = advantage.pow(2).mean()                         # Minimize advantage**2, acting optimally means that advantage should be 0
        actor_loss = (-logp_a_history.sum(axis=1) * advantage).mean() # ∇_θ(𝓙(π_θ)) = 𝔼[∇_θ(log π_θ(a_t | s_t) * A(s_t, a_t))]
